...
- 当前状态：Flask 订阅型 MVP 和项目记忆文档均已同步到 GitHub `main`；本地 `.env` 字段已补齐，但生产 `DATABASE_URL`、`APP_BASE_URL`、`STRIPE_WEBHOOK_SECRET` 仍需真实值。

## 必读文件
- `AGENTS.md`：项目协作规则和记忆流程。
- `NEXT_TASKS.md`：当前最高优先级和下一步。
- `PROGRESS.md`：已完成内容、验证结果和已知阻塞。

## 按需读取
- `PROJECT_CONTEXT.md`：涉及项目结构、模块关系、启动方式时读取。
- `DECISIONS.md`：涉及技术版本、API、上传策略、安全策略时读取。

## 关键注意
- `stock_alert_project/.env` 存在真实密钥，只能本地使用，禁止上传或回显。
- GitHub 已确认 `.env` 未上传，远端回读 `.env` 返回 404。
- 2026-06-17 已补齐 `.env` 缺失 key：`APP_BASE_URL`、`SECRET_KEY`、`DATABASE_URL`；`SECRET_KEY` 已生成强随机值。
- 当前 `.env` 的 `DATABASE_URL` 仍是 MySQL 占位连接串，直接启动 Flask 时 `database_ready=false`，需要替换为真实 MySQL 连接。
